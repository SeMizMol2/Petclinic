...
const isConfirmedAppointmentStatus = (status) => {
  const text = normalizeText(status);
  return text.includes('ยืนยัน') || text.includes('à¸¢à¸·à¸™à¸¢à¸±à¸™');
};
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const paidReceipts = revenueResult.rows.filter((row) =>
      isPaidReceiptStatus(row.payment_status)
    );
    const totalTreatmentAmount = treatmentsResult.rows.reduce(
    ...
    const confirmedAppointments = appointmentsResult.rows.filter((row) =>
      isConfirmedAppointmentStatus(row.appt_status)
    ).length;
    const canceledAppointments = appointmentsResult.rows.filter((row) =>
      isCanceledAppointmentStatus(row.appt_status)
    ).length;
